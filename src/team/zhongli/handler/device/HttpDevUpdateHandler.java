 ...
 *
 */
public class HttpDevUpdateHandler implements HttpHandler {
	...
	private String createResponseFromQueryParams(URI uri) throws Exception {
		...
		if (query != null) {
			...
			dev = devDao.queryById(params.get("deviceip"));
			dev.setDevicename(params.get("newname"));
			devDao.updateDevice(dev);
			if (dev == null) {
				return "fail";
			}
		}
